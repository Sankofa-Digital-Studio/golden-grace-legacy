...
import { Play, Pause, Volume2, VolumeX, Square, Activity, Info } from 'lucide-react';
...
/**
 * --- VIDEO MODAL v4.3.1 ---
 * Purpose: Immersive documentary viewing experience.
 * Fix: Resolved syntax error and aligned with named export conventions.
 */
export const VideoModal = ({ videoSrc, poster, onClose }) => {
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <p className="text-amber-500 text-[10px] font-black uppercase tracking-[0.4em]">
                Movement: {activeChapter.title}
              </p>
            </div>
            <h3 className="text-white font-serif italic text-lg opacity-90">
              {activeChapter.desc}
            </h3>
          </div>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 z-30 bg-gradient-to-t from-black via-black/60 to-transparent">
          ...
          <div className="flex gap-1.5 mb-6 h-1">
            {VIDEO_CHAPTERS.map((chapter, idx) => {
              const nextTime = VIDEO_CHAPTERS[idx + 1]?.time || 108;
              const chapterWidth = ((nextTime - chapter.time) / 108) * 100;
              const isPassed = progress >= (chapter.time / 108) * 100;
      ...
      <div className="fixed bottom-10 left-0 right-0 flex flex-col items-center gap-6 z-[1100]">
        <div className="max-w-xs text-center px-6 opacity-0 animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-500">
          <p className="text-[10px] text-gray-500 italic">
            "Observe the manual, heat-guarded process from the heart of the Free State."
          </p>
        </div>
