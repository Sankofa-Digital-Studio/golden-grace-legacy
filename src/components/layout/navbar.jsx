...
const Navbar = ({ isScrolled, toggleCart, cartCount, activePage, navigate }) => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled || isOpen || activePage !== 'home' ? 'bg-[#050505]/95 backdrop-blur-xl py-3 md:py-4 border-b border-white/5' : 'bg-transparent py-4 md:py-6 lg:py-8'}`}
      >
        <div className="max-w-[1920px] mx-auto px-4 md:px-8 lg:px-12 flex justify-between items-center">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="font-serif text-lg md:text-xl lg:text-2xl tracking-widest leading-none text-white shadow-black drop-shadow-md">
                GOLDEN
              </span>
              <span className="text-amber-500 font-sans text-[0.6rem] md:text-xs lg:text-sm tracking-[0.35em] font-bold shadow-black drop-shadow-md">
                GRACE
              </span>
            </div>
          </div>
          <div className="hidden lg:flex items-center gap-8 xl:gap-12 2xl:gap-16">
            {navLinks.map((item) => (
              ...
              >
                {item.label}
                <span
                  className={`absolute -bottom-2 left-0 h-px bg-amber-400 transition-all duration-300 ${activePage === item.id ? 'w-full' : 'w-0 group-hover:w-full'}`}
                ></span>
              </button>
            ))}
          </div>
          <div className="flex items-center gap-4 md:gap-6 z-50 relative">
            <button
              onClick={toggleCart}
              className="text-white hover:text-amber-400 transition-colors relative group drop-shadow-md"
            >
      ...
      {/* Mobile Menu - Left Aligned with Honeycombs */}
      <div
        className={`fixed inset-0 bg-[#080808] z-40 flex flex-col justify-center px-8 transition-all duration-500 ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full pointer-events-none'}`}
      >
        ...
        <div className="flex flex-col gap-6 z-10 w-full max-w-md">
          <p className="text-gray-500 text-xs uppercase tracking-[0.2em] mb-4 border-b border-white/10 pb-2">
            Menu
          </p>
          {navLinks.map((item, i) => (
            <button
            ...
            >
              <Hexagon
                size={16}
                className={`flex-shrink-0 ${activePage === item.id ? 'text-amber-400 fill-amber-400' : 'text-white/20'}`}
              />
              {item.label}
            </button>
          ))}
