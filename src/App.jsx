...
import ImpactTracker from './sections/impact-tracker';
...
const App = () => {
  ...
  const handleCheckout = () => {
    if (cart.length > 0) setToast({ message: 'Proceeding to secure checkout...', type: 'success' });
  };
    ...
    if (activeModal === 'sustainability') {
      ...
      return (
        <div className="text-center">
          <div className="bg-amber-500/10 p-6 rounded-full w-24 h-24 mx-auto mb-4 flex items-center justify-center border-2 border-amber-500">
            <span className="text-3xl font-bold text-amber-500">{impactScore}</span>
          </div>
  ...
  return (
    <ErrorBoundary>
      <div className="bg-[#050505] min-h-screen text-white font-sans selection:bg-amber-500 selection:text-black overflow-x-hidden">
        ...
        {activeModal && (
          <Modal
            title={activeModal === 'review' ? 'Share Your Experience' : 'Information'}
            content={
              activeModal === 'review'
                ? 'Form placeholder: Name, Rating, Comments...'
                : 'Content loading...'
            }
        ...
        {/* Cart Drawer */}
        <div
          className={`fixed inset-y-0 right-0 w-full md:w-96 bg-[#1a1a1a] z-[60] transform transition-transform duration-500 border-l border-white/10 shadow-2xl p-6 md:p-8 flex flex-col ${showCart ? 'translate-x-0' : 'translate-x-full'}`}
        >
            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-6">
                {cart.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-4 animate-fade-in-up bg-white/5 p-3 rounded-lg border border-white/5"
                  >
            ...
            <div className="mt-8 pt-6 border-t border-white/10">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-2xl font-serif text-white">
                  R {cart.reduce((acc, item) => acc + item.price * item.qty, 0).toFixed(2)}
                </span>
              </div>
              <button
