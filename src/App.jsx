...
const App = () => {
  ...
  // Save Impact Score
  useEffect(() => { localStorage.setItem('impactScore', impactScore); }, [impactScore]);
  ...
  const renderPage = () => {
      switch(activePage) {
          ...
          case 'education': return <BeeSmartPage onScheduleClick={() => setActiveModal('schedule')} />;
      ...
      if (activeModal === 'sustainability') {
          ...
          return (
              <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500 mb-8">{nextLevel - impactScore} more jars to reach next level</p>
                  
                  <button onClick={handleLogReturn} className="flex items-center justify-center gap-2 w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded transition-colors uppercase text-xs tracking-widest">
                      <Recycle size={16} /> Log Jar Return (+1)
                  </button>
                  <p className="text-[10px] text-gray-500 mt-4">* Returns must be verified in-store for point redemption.</p>
              </div>
          );
      }
