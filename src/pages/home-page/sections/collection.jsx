...
const Collection = ({ onAddToCart }) => {
  ...
  return (
    <Section id="collection" className="overflow-hidden">
      {/* Background Atmosphere (full-bleed, behind container) */}
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-5 md:px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95
                ${
                  active
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {filteredProducts.map((product) => {
          const selected = selectedSizes[product.id] || product.sizes?.[0];
              ...
              <div className="space-y-2 mb-6">
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0">
                    <p className="text-amber-500 text-[9px] font-black uppercase tracking-[0.3em] mb-1">
                      {product.flora}
                    </p>
                    <h4 className="text-white font-serif text-2xl group-hover:text-amber-400 transition-colors truncate">
                      {product.title}
                    </h4>
                  </div>
                  <p className="text-white font-serif text-xl shrink-0">R{product.price}</p>
                </div>
      ...
      {/* 4) Footer note */}
      <div className="mt-16 md:mt-20 flex flex-col items-center gap-4 py-10 md:py-12 border-t border-white/5">
